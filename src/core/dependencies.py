...
class DependencyContainer:
    ...
    async def get_embedder(self) -> Any:
        ...
        logger.info("get_embedder called, checking if _embedder is None")
        if self._embedder is None:
            logger.info("_embedder is None, initializing...")
            try:
                logger.info(f"About to call get_embedder with: {settings.embedder_name}")
                self._embedder = get_embedder(settings.embedder_name)
                logger.info(f"Successfully got embedder instance")
    ...
    async def get_ingestor(self) -> DocumentIngestor:
        ...
        if self._ingestor is None:
            try:
                # Initialize the ingestor with shared components to avoid creating new connections
                ...
                # Initialize with shared components instead of creating new ones
                await self._ingestor.initialize_with_shared_components(
                    embedder=await self.get_embedder(),
                    vector_store=await self.get_async_vector_store()
                )
    ...
    async def initialize_all(self) -> None:
        ...
        async with self._initialization_lock:
            ...
            try:
                ...
                logger.info("1. Initializing embedder...")
                logger.info("About to await self.get_embedder()")
                await self.get_embedder()
                logger.info("1. ✓ Embedder initialized")
                
                logger.info("2. Initializing async vector store...")
                await self.get_async_vector_store()  # Initialize async vector store only
                logger.info("2. ✓ Async vector store initialized")
                
                # Skip separate sync vector store to reduce connections
                logger.info("3. ✓ Vector store (using async store)")
                
                logger.info("4. Initializing redis cache...")
                # Temporarily skip Redis to avoid blocking
                # await self.get_redis_cache()         # Initialize caching (optional)
                logger.info("4. ✓ Redis cache initialized (skipped for debugging)")
                
                logger.info("5. Initializing ingestor...")
                await self.get_ingestor()
                logger.info("5. ✓ Ingestor initialized")
                
                logger.info("6. Initializing auth manager...")
                await self.get_auth_manager()
                logger.info("6. ✓ Auth manager initialized")
                
                logger.info("7. Initializing chat handler...")
                await self.get_chat_handler()
                logger.info("7. ✓ Chat handler initialized")
                
                logger.info("8. Initializing health handler...")
                await self.get_health_handler()
                logger.info("8. ✓ Health handler initialized")
