...
def cmd_start(
    ...
    if os.path.exists(setup_env_script):
        if mode == "dev":
            ...
            _run(f"{setup_env_script} up -d qdrant api ui", cwd=REPO_ROOT)
        else:
        ...
        if mode == "dev":
            _run("docker compose up -d qdrant api ui", cwd=REPO_ROOT)
        else:
...
def cmd_stop() -> None:
    ...
    typer.secho("Qdrant (localhost:6333):", fg=typer.colors.BLUE)
    if _is_qdrant_running():
        typer.secho("reachable", fg=typer.colors.GREEN)
    else:
        typer.secho("unreachable", fg=typer.colors.RED)
...
def cmd_ingest(
    ...
    cmd = ["docker", "compose", "run", "--rm", "ingest"]
    if path:
...
def cmd_flush(force: bool = typer.Option(False, "--yes", "-y", help="Skip confirmation")) -> None:
    ...
    if not _is_qdrant_running():
        typer.secho("Qdrant is not running – start the stack first.", fg=typer.colors.RED)
        raise typer.Exit(code=1)

    if not force and not typer.confirm("Delete ALL Qdrant collections? This action is irreversible."):
        raise typer.Exit()
...
def _is_qdrant_running() -> bool:
    """Check if Qdrant HTTP endpoint is reachable."""
    try:
        result = subprocess.run(
