...
# Fixtures for Qdrant container are no longer needed, as we now use mocks
# Leaving comments for documentation of historical context
...
async def api_client(monkeypatch: MonkeyPatch) -> AsyncGenerator[httpx.AsyncClient, None]:
    """
    Provides an httpx.AsyncClient for making API requests to the test app.
    This fixture also handles mocking external API calls (Jina, OpenRouter).
    """
    ...
    mock_qdrant.query_points = _fake_query_points
    
    # Ensure application code uses the mocked Qdrant client
    import root.app as sentio_app  # Local import to avoid circularity at top level
    sentio_app.qdrant_client = mock_qdrant
